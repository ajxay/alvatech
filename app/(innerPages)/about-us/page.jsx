import Header3 from "@/components/headers/Header3";

import Footer1 from "@/components/footers/Footer1";
import About from "@/components/innerpages/About";
import AboutMission from "@/components/innerpages/AboutMission";
import AboutWhyChoose from "@/components/innerpages/AboutWhyChoose";
import About2 from "@/components/innerpages/About2";
import AboutValues from "@/components/innerpages/AboutValues";
import AboutTechStack from "@/components/innerpages/AboutTechStack";
import TrustedClients from "@/components/homes/home-13/TrustedClients";
import Faq from "@/components/homes/home-13/Faq";
import Cta from "@/components/innerpages/Cta";
import AboutTeam from "@/components/innerpages/AboutTeam";
import { aboutFaq } from "@/data/faq";
import { pageMetadataFor } from "@/data/pageMeta";
import "@/components/innerpages/AboutPage.css";

export const metadata = pageMetadataFor("/about-us");

export default function AboutPage() {
  return (
    <>
      <div className="about-page page-wrapper uni-body panel bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-200 overflow-x-hidden bp-xs bp-sm bp-md bp-lg bp-xl bp-xxl dom-ready">
        <Header3 />

        <div id="wrapper" className="wrap">
          <About />
          <About2 />
          <AboutMission />
          <AboutWhyChoose />
          <AboutTeam />
          <AboutTechStack />
          <TrustedClients />
          <Faq
            id="about-faq"
            faqData={aboutFaq}
            titleKey="pages.about.faq.title"
          />
          <Cta />
        </div>
      </div>
      <Footer1 />
    </>
  );
}
